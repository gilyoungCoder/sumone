-- =============================================
-- FIX: Align DB schema to match app code
-- Run this in Supabase SQL Editor:
-- https://supabase.com/dashboard/project/yybiqpidheapmwvwfqjn/sql/new
-- =============================================

-- 1. Fix questions table: rename columns to match app code
DO $$
BEGIN
  -- Rename 'text' -> 'question_text' if needed
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'questions' AND column_name = 'text'
  ) THEN
    ALTER TABLE questions RENAME COLUMN text TO question_text;
  END IF;

  -- Rename 'sort_order' -> 'question_number' if needed
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'questions' AND column_name = 'sort_order'
  ) THEN
    ALTER TABLE questions RENAME COLUMN sort_order TO question_number;
  END IF;
END $$;

-- 2. Fix daily_questions table: add inline answer columns if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'daily_questions' AND column_name = 'user1_id'
  ) THEN
    ALTER TABLE daily_questions ADD COLUMN user1_id UUID REFERENCES auth.users(id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'daily_questions' AND column_name = 'user2_id'
  ) THEN
    ALTER TABLE daily_questions ADD COLUMN user2_id UUID REFERENCES auth.users(id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'daily_questions' AND column_name = 'user1_answer'
  ) THEN
    ALTER TABLE daily_questions ADD COLUMN user1_answer TEXT;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'daily_questions' AND column_name = 'user2_answer'
  ) THEN
    ALTER TABLE daily_questions ADD COLUMN user2_answer TEXT;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'daily_questions' AND column_name = 'user1_answered_at'
  ) THEN
    ALTER TABLE daily_questions ADD COLUMN user1_answered_at TIMESTAMPTZ;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'daily_questions' AND column_name = 'user2_answered_at'
  ) THEN
    ALTER TABLE daily_questions ADD COLUMN user2_answered_at TIMESTAMPTZ;
  END IF;
END $$;

-- 3. Drop old 'answers' table if it exists (answers are now inline)
DROP TABLE IF EXISTS answers;

-- 4. Seed 50 English questions (skip if already seeded)
INSERT INTO questions (question_text, category, question_number)
SELECT * FROM (VALUES
...
) AS v(question_text, category, question_number)
